...
That is, the CRU CL v. 2.0 data files *must* be downloaded prior to the use of these functions using a program external to R, _e.g._, FileZilla or some other FTP program.
In this instance it is recommended to use an FTP client (_e.g._, [FileZilla](https://filezilla-project.org)) to download the files, save locally and use one of these functions to import the data into R and generate your desired object to work with.
