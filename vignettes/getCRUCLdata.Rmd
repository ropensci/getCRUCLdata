...
There are two methods that this package provides for creating tidy data frames of the CRU CL2.0 climate data. Depending on your needs you may need to use `create_CRU_df()`, which will create a tidy data frame of the climate elements from *locally available* files. Or you can use `get_CRU_df()`, which will automate the fetching and importing of the data files into R in one step.
