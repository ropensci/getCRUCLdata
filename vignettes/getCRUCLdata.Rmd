...
frames as a `tibble::tibble()` of the CRU CL v. 2.0 climatology elements. However,
it is not required that _tibble_ even be installed on your system to use the
package. It will work as a normal `data.frame` if _tibble_ is not installed or
loaded in your current R session.
...
Now that we have the data, we can plot it easily using _ggplot2_ and the
_viridis_ package for the colour scale.
