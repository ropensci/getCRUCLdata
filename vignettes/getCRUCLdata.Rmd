...
[tidy data frame](http://vita.had.co.nz/papers/tidy-data.html) as a [`tibble::tibble()`](https://www.rdocumentation.org/packages/tibble/versions/1.2)
or a
...
frames as a [_tibble_](https://github.com/tidyverse/tibble) of the CRU CL v. 2.0
...
[_ggplot2_](http://ggplot2.org) and the 
[_viridis_](https://CRAN.R-project.org/package=viridis) package for the colour
scale.
...
lists of [_raster_](https://CRAN.R-project.org/package=raster) stacks of the
data.
...
available, see help for `writeRaster()` and `writeFormats()` for more options)
in the `Data` directory with a tmn or tmx prefix to the month for a file name.
...
(e.g., [FileZilla](https://filezilla-project.org)) to download the files, save
